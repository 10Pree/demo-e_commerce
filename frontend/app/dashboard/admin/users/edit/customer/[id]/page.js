...
export default function Page() {
    ...
    const [passwordUpdate, setPasswordUpdata] = useState(false)
    ...
    const update = async (e) => {
        ...
        try {
            ...
            const res = await axios.put(`http://localhost:8000/customer/${userId}`, fromData, { withCredentials: true, headers: { "Content-Type": "multipart/form-data" } })
            ...
            Swal.fire({
                ...
                text: "อัพเดตไม่สำเร็จ กรุณาลองใหม่",
                timer: 2000
            })
        } finally {
            setLoading(false)
        ...
        <form onSubmit={update}>
            ...
            <div className="flex flex-col justify-center items-center">
                <div className="w-full md:w-[50%]">
                    ...
                    <div className="text-end"><button type="submit" className="bg-[#1E3A8A] px-8 py-2 rounded-2xl text-white mt-0 md:mt-16">{loading ? "กำลังโหลด..." : "บันทึก"}</button></div>
                </div>
            </div>
        </form>
    )
