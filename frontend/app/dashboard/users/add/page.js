...
export default function Page() {
    ...
    const addUser = async() => {
        try{
            const res = await axios.post('http://localhost:8000/user', data, {withCredentials:true})
        }catch(err){
    ...
    const handleUpload = (e) => {
        try{
            ...
            const filename = image.map((img) => "/uploads/users/" + img.name)
            seturlImagePreview(preview)
            console.log(preview)
    ...
    return (
        <form>
            {
                showPopupAdd && <FromAdd onClose={handleClosePopup} password={setdata}/>
            }
            ...
            <div className="w-full h-full flex-row justify-center items-center gap-4 md:flex md:w-full">
                <div className="w-full h-1/2 bg-white rounded-2xl shadow-2xl p-4 md:w-1/2">
                    ...
                    <div>
                        ...
                        <input className="bg-white border-[1px] rounded-[8px] p-1 w-full" type="number"  name="phone" onChange={handleChange} placeholder="012-314-5678"/>
                    </div>
                    <div>
